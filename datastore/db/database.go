...
// Database is the primary database for the application.
type Database struct {
	*sqlx.DB
}

// SetupDatabase attempts configure and connect to the database
func SetupDatabase(lc fx.Lifecycle, config Settings, log *zap.Logger) (Database, error) {
	db, err := open(config)
	if err != nil {
		return Database{}, fmt.Errorf("unable to establish a connection with the db: %w", err)
	}
	...
	if err != nil {
		return Database{}, fmt.Errorf("database migrations failed: %w", err)
	}
